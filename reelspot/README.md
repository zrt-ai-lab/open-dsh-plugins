...
- 🖥️ **操作者监视窗** — 确认分享后**自动弹出** Document PiP 悬浮小窗（可拖动/缩放），全程常驻显示录制画面，窗内叠加取景小地图（黄框标出当前取景范围）和实时倍数；这些辅助标记**只存在于监视窗，绝不进入成片**。录**标签页**和录**窗口**时监视窗都拍不到（它是独立顶层窗口）；录**整个屏幕**时默认不自动弹（会被录进去），需要时点 🖥️ 手动打开并拖到副屏
